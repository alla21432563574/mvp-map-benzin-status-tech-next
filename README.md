...
SCRAPER_REPORTS_MODE=incremental
...
После снимка станций scraper запрашивает публичную карточку `/api/stations/{id}` и импортирует реальные ID отметок, статусы, виды топлива, комментарии и GPS-признак `near`; отсутствующие комментарии и непривязанная к отметке очередь не синтезируются. В обычном `SCRAPER_REPORTS_MODE=incremental` карточка запрашивается только у АЗС, где `lastReportAt` новее персонального курсора в `station_report_sync` или попадает в окно `SCRAPER_REPORT_LOOKBACK_HOURS`.

Для первоначального полного импорта публично видимых отметок используйте `SCRAPER_REPORTS_MODE=backfill`. В этом режиме станции без курсора тоже попадают в очередь detail-запросов, а из их карточек импортируются все публично возвращаемые донором отметки. `SCRAPER_MAX_REPORT_STATIONS` ограничивает число карточек за один запуск, поэтому backfill идёт постепенно и не создаёт резкой нагрузки на donor-сайт. Уже обработанные станции получают курсор; после завершения backfill режим фактически работает как incremental и берёт только новые карточки. Не поместившиеся станции остаются без сдвига курсора и автоматически обрабатываются в следующих запусках. Повторные ID отбрасываются уникальным ключом `(source, external_id)` в `station_reports`.
